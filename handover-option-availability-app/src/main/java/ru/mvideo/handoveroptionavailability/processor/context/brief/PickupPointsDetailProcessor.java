...
public class PickupPointsDetailProcessor extends BaseProcessor<BriefAndPickupContext> {
	...
	protected Mono<BriefAndPickupContext> executeProcessor(BriefAndPickupContext context) {
		return Mono.defer(() -> {
			...
			return pickupPointsService.detailPickupPoints(rimCode, brand, availabilityOptions, products)
					.doOnSuccess(pickupPoints -> {
						...
						context.detailPickupPoints(pickupPoints);
